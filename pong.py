...
# табло
score_1 = pygame.font.Font(None, 30)
score_1_img = score_1.render(str(player1_score), True, WHITE)
score_2 = pygame.font.Font(None, 30)
...
while game:
    ...
    if keys[pygame.K_UP]:  # клавиша стрелка вверх
        if player2.y >= 0:
            player2.y -= 30  # двигаем игрока вверх (в PG Y растет вниз)
    if keys[pygame.K_DOWN]:  # клавиша стрелка вниз
        if player2.y <= (screen_height - player_height2):
            player2.y += 30  # двигаем игрока вниз
    if keys[pygame.K_w]:  # клавиша стрелка вверх
        if player1.y >= 0:
            player1.y -= 30  # двигаем игрока вверх (в PG Y растет вниз)
    if keys[pygame.K_s]:  # клавиша стрелка вниз
        if player1.y <= (screen_height - player_height2):
            player1.y += 30  # двигаем игрока вниз

    # логика
    ball.x += ball_speed_x  # движение мяча по X
    ball.y += ball_speed_y
    if ball.y < 0:  # отскок
        ball_speed_y *= -1
    ...
    # отскок от ракетки
    if ball.colliderect(player1) or ball.colliderect(player2): #  мяч касается в игроков  
        ball_speed_x *= -1
    ...
    score_1_img = score_1.render(str(player1_score), True, WHITE) #снимаем с табло картинку
    score_2_img = score_2.render(str(player2_score), True, WHITE)
    screen.blit(score_1_img, (screen_width * 0.25, 50)) #рисуем табло(цифры)
    screen.blit(score_2_img, (screen_width * 0.75, 50)) #рисуем табло(цифры)
